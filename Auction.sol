...
contract nftAuction {
   ...
   function setNftAddress(address _address) public  {
    ...
    function revoke(uint256 _tokenId) public payable{                // this should be called by the owner of token to revoke the bid.
          require(msg.sender == nft.ownerOf(_tokenId));
          highestBidder[_tokenId].transfer(highestBid[_tokenId]);     
          highestBid[_tokenId] = 0;
          highestBidder[_tokenId] = address(0);
          nft.approveMArketOrGame(msg.sender,address(0),_tokenId);
          ended[_tokenId] = true;
    }
